...
type
  TSynUniFormatNativeXml20 = class(TSynUniFormatNativeXml)
    ...
    {$IFDEF SYN_CodeFolding}
    class function ImportCodeFolding(AFoldRegions: TSynFoldRanges; ANode: IXMLNode): Boolean;
    class function ExportCodeFolding(AFoldRegions: TSynFoldRanges; ANode: IXMLNode): Boolean;
    {$ENDIF}
    {$IFDEF CUSTOMDATA}
...
{$IFDEF SYN_CodeFolding}
//----------------------------------------------------------------------------
class function TSynUniFormatNativeXml20.ImportCodeFolding(AFoldRegions: TSynFoldRanges; ANode: IXMLNode): Boolean;
var
  i: integer;
  NewRegion: TFoldRegionItem;
//  FoldRegionList: TSynFoldRanges;
begin
{  if ParentRegion = nil then
  ...
  begin
    with ANode.ChildNodes.Item[i] do begin
      // Modified by Administrator 2007-12-1 ЙПОз 12:09:48
...
//----------------------------------------------------------------------------
class function TSynUniFormatNativeXml20.ExportCodeFolding(AFoldRegions: TSynFoldRanges; ANode: IXMLNode): Boolean;
var
  i: integer;
  NewNode: IXMLNode;
//  FoldRegionList: TSynFoldRanges;
...
{  if ARegion = nil then
  ...
  // Modified by Administrator 2007-12-1 ЙПОз 12:10:57
...
begin
  ...
  begin
    ...
    with AInfo.Author, EnsureChild('Author') do begin
      Name      := VarToStr(GetVarAttr('Name',''));
      Email     := VarToStr(GetVarAttr('Mail','')); //TODO: Переименоват?обратн??'Email', блин...
...
begin
  ...
  begin
    ...
    begin
      SetVarAttr('Name', Name);
      SetVarAttr('Mail', Email); //TODO: Переименоват?обратн??'Email', блин...
      SetVarAttr('Web', Web);
...
begin
  ...
  begin
    ParentForeground := False; // Нужн?ли?..
...
begin
  ...
  begin
    for i := 0 to Count-1 do
    begin
      if Names[i] <> '' then //TODO: не позволять дубликатов по Name
      begin
        Node := AppendElement('Entry');
        Node.SetAttr('Name', Names[i]);
        Node.SetAttr('Value', Values[Names[i]]);
      end;
    end;
  end;
  Result := True;
...
begin
  ...
  begin
    ...
    {$IFDEF SYN_CodeFolding}ImportCodeFolding(FoldRegions, EnsureChild('CodeFolding'));{$ENDIF}
    {$IFDEF CUSTOMDATA}ImportCustomData(CustomData, EnsureChild('CustomData'));{$ENDIF}
    FormatVersion := '2.0';
  end;
  Result := True;
end;
...
begin
  ...
  begin
    ...
    {$IFDEF SYN_CodeFolding}ExportCodeFolding(FoldRegions, ANode.AppendElement('CodeFolding'));{$ENDIF}
    {$IFDEF CUSTOMDATA}ExportCustomData(CustomData, ANode.AppendElement('CustomData'));{$ENDIF}
  end;
  Result := True;
end;
