...
Backported to the classic (Delphi 7 compatible) SynEdit code base from the
PyScripter fork of SynEdit.
...
{ Counts the leading white space characters of S (tabs count as one). }
...
begin
  inherited;
  NewIndent := LeadingSpaces(FLineStr);
  ...
  end else if (Copy(FLineStr, 1, 3) = sDocStart) or
    (Copy(FLineStr, 1, 3) = sDocEnd) then
  begin
...
begin
  ...
  if TryStrToFloat(WideTrim(Val), FloatVal) then
...
begin
  Result := SynIsCharAlphaNumeric(AChar) or (AChar = '_');
end;
