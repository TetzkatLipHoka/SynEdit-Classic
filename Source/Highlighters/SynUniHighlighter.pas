...
type
  ...
  protected
    ...
    function GetAdditionalIdentChars(): TSynIdentChars;
  ...
  public
    ...
    function GetToken(): UnicodeString; override; // Current token (string from fTokenPos to Run)
    ...
    procedure SetLine(const NewValue: UnicodeString; LineNumber: Integer); override; { Set current line in SynEdit for highlighting }
    ...
    //procedure CreateStandardRules(); вернут?
...
//----------------------------------------------------------------------------
procedure TSynUniSyn.SetLine(const NewValue: UnicodeString; LineNumber: Integer);
begin
...
begin
  ...
  begin // If we can't find token from current position
    //TODO: возможно стои?запихнут?эт??TDefaultParser (дл€ нагл€дности) 
    ...
    else // !!! можн?вставить свойство, типа если один range закрываетс€ ?сраз?открываетс€ другой... “огд?этог?else не буде?..
...
begin
  if (fDefaultFilter = '') and
    (Info.General.Name <> '') and
    (Info.General.Extensions <> '') then
  begin
    fDefaultFilter := Info.General.Name + ' Files(*.';
    y := TrimRight(TrimLeft(Info.General.Extensions));
    ...
    fDefaultFilter := fDefaultFilter + t + ')|*.' + t + ';';
  end;
...
//----------------------------------------------------------------------------
function TSynUniSyn.GetToken(): UnicodeString;
var
  Len: LongInt;
begin
  Len := Run - fTokenPos;
...
//----------------------------------------------------------------------------
function TSynUniSyn.GetAdditionalIdentChars(): TSynIdentChars; // Get word break chars
begin
  Result := [#32..#255] - CurrRange.Delimiters;
end;

//----------------------------------------------------------------------------
function TSynUniSyn.GetTokenKind(): Integer; //TODO: «аюзат?дл€ каки?нибудь целе?
begin
  Result := 1; //TODO: Ѕрат?индекс стил€ ;)
end;
...
//----------------------------------------------------------------------------
procedure TSynUniSyn.SetSampleSource(Value: UnicodeString);
//: Set sample text
begin
...
//----------------------------------------------------------------------------
function TSynUniSyn.GetLanguageNameProp: string;
begin
...
//----------------------------------------------------------------------------
function TSynUniSyn.LoadFromFile(AFileName: string): Boolean;
begin
  result := TSynUniFormatNativeXmlAuto.ImportFromFile(Self, AFileName);
end;
...
//----------------------------------------------------------------------------
function TSynUniSyn.SaveToFile(AFileName: string): Boolean;
begin
  result := TSynUniFormatNativeXml20.ExportToFile(Self, AFileName);
end;
