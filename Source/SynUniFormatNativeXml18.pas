...
begin
  Result := ImportAttributes(Attributes, ANode.GetAttr('Attributes',''));
end;
...
begin
  ...
  begin
    ...
    with NodeRule do
    begin
      if (OpenToken.SymbolCount > 0) and (OpenToken.Symbols[0] <> '') then
        SetVarAttr('OpenSymbol', OpenToken.Symbols[0]);
      ExportToken(OpenToken, NodeRule, 'OpenSymbol');
      if (CloseToken.SymbolCount > 0) and (CloseToken.Symbols[0] <> '') then
