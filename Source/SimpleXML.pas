{************************************************************
 SimpleXML - Библиотека для синтаксическог?разбор?тексто?XML
	 ?преобразован? ?иерархию XML-объектов.
	 ?наоборот: можн?сформировать иерархию XML-объектов, ?
	 уж?из не?получить текс?XML.
	 Достойная замена для MSXML. Пр?использовани?Ansi-стро?
	 работает быстре??кушает меньше па?ти.

 (? Авторски?прав?2002,2003 Михаил Власов.
	 Библиотека бесплатн? ?може?быть использована по любому назначению.
	 Разрешается внесение любы?изменени??использовани?измененных
	 библиоте?бе?ограничени?
	 Единственное требование: Данный текс?должен присутствовать
	 бе?изменени?во всех модификация?библиотеки.

	 Вс?пожелания приветству?по адресу misha@integro.ru
	 Та?же рекомендую посетить мо?страничк? http://mv.rb.ru
	 Та?Вы всегда найдет?саму?последню?версию библиотеки.
	 Жела?пр?тног?программирован?, Михаил Власов.
...
type
	// TXmlString - ти?строковы?переменных, используемых ?SimpleXML.
	//  Може?быть String ил?WideString.
	...
	// IXmlBase - базовы?интерфей?для всех интерфейсо?SimpleXML.
	IXmlBase = interface
		// GetObject - возвращает ссылку на объект, реализующи?интерфей?
		function GetObject: TObject;
	end;

	// IXmlNameTable - таблиц?имен. Каждом?имен?сопостав?ет? неки?
	//  уникальный числовой идентификато? Использует? для хранен?
	//  названий тэго??атрибуто?
	IXmlNameTable = interface(IXmlBase)
		// GetID - возвращает числовой идентификато?указанно?строки.
		function GetID(const aName: TXmlString): Integer;
		// GetID - возвращает строку, соответствующу?указанному числовом?
		//  идентификатору.
	...
	// IXmlNodeList - список узло? Список организова??виде массив?
	//  Доступ ?элемента?списка по индекс?
	IXmlNodeList = interface(IXmlBase)
		// Get_Count - количество узло??списке
		function Get_Count: Integer;
		// Get_Item - получить узел по индекс?
		function Get_Item(anIndex: Integer): IXmlNode;
		// Get_XML - возвращает представлени?элементо?списка ?формат?XML
	...
	IXmlNode = interface(IXmlBase)
		// Get_NameTable - таблиц?имен, используем? данным узло?
		function Get_NameTable: IXmlNameTable;
		// Get_NodeName - возвращает название узла. Интерпретация назван? узла
		//  зависи?от ег?типа
		function Get_NodeName: TXmlString;
		// Get_NodeNameID - возвращает ко?назван? узла
		function Get_NodeNameID: Integer;
		// Get_NodeType - возвращает ти?узла
		function Get_NodeType: Integer;
		// Get_Text - возвращает текс?узла
		function Get_Text: TXmlString;
		// Set_Text - изме?ет текс?узла
		procedure Set_Text(const aValue: TXmlString);
		// Get_DataType - возаращает ти?данных узла ?терминах варианто?
		function Get_DataType: Integer;
		// Get_TypedValue - возвращает 
		function Get_TypedValue: Variant;
		// Set_TypedValue - изме?ет текс?узла на типизированное значение
		procedure Set_TypedValue(const aValue: Variant);
		// Get_XML - возвращает представлени?узла ?всех вложенны?узло?
		//  ?формат?XML.
		function Get_XML: TXmlString;

		// CloneNode - создае?точную копи?данног?узла
		//  Если зада?призна?aDeep, то создастся копия
		//  всей ветв?иерархии от данног?узла.
		...
		//  ?которо?расположен данный узел
		function Get_OwnerDocument: IXmlDocument;

		// Get_ChildNodes - возвращает список дочерних узло?
		function Get_ChildNodes: IXmlNodeList;
		// AppendChild - добавляет указанны?узел ?коне?списка дочерних узло?
		procedure AppendChild(const aChild: IXmlNode);
		// InsertBefore - добавляет указанны?узел ?указанно?мест?списка дочерних узло?
		procedure InsertBefore(const aChild, aBefore: IXmlNode);
		// ReplaceChild - заме?ет указанны?узел другим узло?
		procedure ReplaceChild(const aNewChild, anOldChild: IXmlNode);
		// RemoveChild - удаляет указанны?узел из списка дочерних узло?
		procedure RemoveChild(const aChild: IXmlNode);

		// AppendElement - создае?элемен??добавляет ег??коне?списка
		//  ?коне?списка дочерних объектов
		...
		// AppendText - создае?текстовы?узел ?добавляет ег?
		//  ?коне?списка дочерних объектов
		function AppendText(const aData: TXmlString): IXmlText; 

		// AppendCDATA - создае?секцию CDATA ?добавляет ее 
		//  ?коне?списка дочерних объектов
		function AppendCDATA(const aData: TXmlString): IXmlCDATASection;

		// AppendComment - создае?комментари??добавляет ег?
		//  ?коне?списка дочерних объектов
		function AppendComment(const aData: TXmlString): IXmlComment; 

		// AppendProcessingInstruction - создае?инструкцию ?добавляет её
		//  ?коне?списка дочерних объектов
		...
		// GetChildText - возвращает значение дочернег?узла
		// SetChildText - добавляет ил?изме?ет значение дочернег?узла
		...
		// NeedAttr - возвращает строково?значение указанного атрибута.
		//  Если атрибу?не зада? то генерирует? исключение
		function NeedAttr(aNameID: Integer): TXmlString; overload;
		function NeedAttr(const aName: TXmlString): TXmlString; overload;

		// GetAttr - возвращает строково?значение указанного атрибута.
		//  Если атрибу?не зада? то возвращает? значение по умолчани?
		// SetAttr - изме?ет ил?добавляет указанны?атрибу?
		...
		// GetBoolAttr - возвращает целочисленно?значение указанного атрибута
		// SetBoolAttr - изме?ет ил?добавляет указанны?атрибу?целочисленны?
		//  значение?
		...
		// GetIntAttr - возвращает целочисленно?значение указанного атрибута
		// SetIntAttr - изме?ет ил?добавляет указанны?атрибу?целочисленны?
		//  значение?
		...
		// GetDateTimeAttr - возвращает целочисленно?значение указанного атрибута
		// SetDateTimeAttr - изме?ет ил?добавляет указанны?атрибу?целочисленны?
		//  значение?
		...
		// GetFloatAttr - возвращает значение указанного атрибута ?виде
		//  вещественног?числ?
		// SetFloatAttr - изме?ет ил?добавляет указанны?атрибу?вещественным
		//  значение?
		...
		// GetHexAttr - получени?значен? указанного атрибута ?целочисленно?виде.
		//  Строково?значение атрибута преобразуется ?цело?числ? Исходн?
		//  строка должна быть задана ?шестнадцатирично?виде бе?префиксо?
		//  ("$", "0x" ?пр.) Если преобразование не може?быть выполнен?
		//  генерирует? исключение.
		//  Если атрибу?не зада? возвращает? значение параметр?aDefault.
		// SetHexAttr - изменени?значен? указанного атрибута на строково?
		//  представлени?целого числ??шестнадцатирично?виде бе?префиксо?
		//		("$", "0x" ?пр.) Если преобразование не може?быть выполнен?
		//		генерирует? исключение.
		//		Если атрибу?не бы?зада? до он буде?добавлен.
		//		Если бы?зада? то буде?измене?
		...
		//	GetEnumAttr - ищет значение атрибута ?указанно?списке стро??
		//		возвращает индекс	найденно?строки. Если атрибу?зада?но не найден
		//		?списке, то генерирует? исключение.
		//		Если атрибу?не зада? возвращает? значение параметр?aDefault.
	...
	IXmlElement = interface(IXmlNode)
		//	ReplaceTextByCDATASection - удаляет вс?текстовы?элементы ?добавляет
		//		одну секцию CDATA, содержащую указанны?текс?
		procedure ReplaceTextByCDATASection(const aText: TXmlString);

		//	ReplaceTextByBynaryData - удаляет вс?текстовы?элементы ?добавляет
		//		один текстовы?элемен? содержащий указанны?двоичные данные
		//		?формат?"base64".
		//		Если параметр aMaxLineLength не раве?нулю, то производит? разбивка
		//		получено?строки на строки длиной aMaxLineLength.
		//		Строки разделяют? паро?символов #13#10 (CR,LF).
		//		Посл?последне?строки указанны?символ?не вставляют?.
		procedure ReplaceTextByBynaryData(const aData; aSize: Integer;
			aMaxLineLength: Integer);

		//	GetTextAsBynaryData - cобирае?вс?текстовы?элементы ?одну строку ?
		//		производит преобразование из формат?"base64" ?двоичные данные.
		//		Пр?преобразовании игнорируют? вс?пробельные символ?(?кодо?<= ' '),
		//		содержащие? ?исходной строке.
...
resourcestring
	SSimpleXmlError1 = 'Ошибка получения элемента списка: индекс выходи?за предел?';
	SSimpleXmlError2 = 'Не завершен?определени?элемента';
	SSimpleXmlError3 = 'Некорретны?символ ?имен?элемента';
	SSimpleXmlError4 = 'Ошибка чтен? двоичног?XML: некорректный ти?узла';
	SSimpleXmlError5 = 'Ошибка записи двоичног?XML: некорректный ти?узла';
	SSimpleXmlError6 = 'Неверное значение атрибута "%s" элемента "%s".'^M^J +
		'Допустимые значен?:'^M^J +
		'%s';
	SSimpleXmlError7 = 'Не найден атрибу?"%s"';
	SSimpleXmlError8 = 'Не зада?атрибу?"%s"';
	SSimpleXmlError9 = 'Данн? возможност?не поддерживает? SimpleXML';
	SSimpleXmlError10 = 'Ошибка: не найден дочерний элемен?"%s".';
	SSimpleXmlError11 = 'Имя должно начинать? ?букв?ил?"_"';
	SSimpleXmlError12 = 'Ожидается числ?';
	SSimpleXmlError13 = 'Ожидается шестнадцатерично?числ?';
	SSimpleXmlError14 = 'Ожидается "#" ил?имя упрамляющег?символ?';
	SSimpleXmlError15 = 'Некорректное имя управляющег?символ?';
	SSimpleXmlError16 = 'Ожидается "%c"';
	SSimpleXmlError17 = 'Ожидается "%s"';
	SSimpleXmlError18 = 'Символ "<" не може?использовать? ?значен??атрибуто?';
	SimpleXmlError19 = 'Ожидается "%s"';
	SSimpleXmlError20 = 'Ожидается значение атрибута';
	SSimpleXmlError21 = 'Ожидается строковая констант?';
	SimpleXmlError22 = 'Ожидается "%s"';
	SSimpleXmlError23 = 'Ошибка чтен? данных.';
	SSimpleXmlError24 = 'Ошибка чтен? значен?: некорректный ти?';
	SSimpleXmlError25 = 'Ошибка записи значен?: некорректный ти?';
...
var
	aPos: Integer;
begin
	if '.' = {$IF CompilerVersion >= 30}FormatSettings.{$ELSE}SysUtils.{$IFEND}DecimalSeparator then
		aPos := Pos(',', s)
	else if ',' = {$IF CompilerVersion >= 30}FormatSettings.{$ELSE}SysUtils.{$IFEND}DecimalSeparator then
		aPos := Pos('.', s)
	else begin
		aPos := Pos(',', s);
		if aPos = 0 then
	...
	if aPos <> 0 then
		s[aPos] := TXmlChar({$IF CompilerVersion >= 30}FormatSettings.{$ELSE}SysUtils.{$IFEND}DecimalSeparator);
	Result := StrToFloat(s);
end;
...
begin
	Result := FloatToStr(v);
	aPos := Pos({$IF CompilerVersion >= 30}FormatSettings.{$ELSE}SysUtils.{$IFEND}DecimalSeparator, Result);
	if aPos <> 0 then
		Result[aPos] := '.';
end;
...
type
	...
	private
		...
		// FNames - таблиц?имен. Задает? извн?
		FNames: TXmlNameTable;
		// Количество атрибуто??массив?FAttrs
		FAttrCount: Integer;
		// Массив атрибуто?
		FAttrs: array of TXmlAttrData;
		// Список дочерних узло?
...
begin
	while not aXML.EOF do begin
		...
		if aXML.CurChar = '<' then // символ разметки
			if aXML.Next then
				if aXML.CurChar = '/' then // закрывающи?тэ?элемента
					Exit
				else if aXML.CurChar = '?' then // инструкц?
					ParseProcessingInstruction
				else if aXML.CurChar = '!' then begin
					if aXML.Next then
						if aXML.CurChar = '-' then // коментарий
							ParseComment
						else if aXML.CurChar = '[' then // секц? CDATA
							ParseCDATA
						else
							ParseDOCTYPE
				end
				else // открывающи?тэ?элемента
					ParseElement
	end;
end;
		...
		for i := 0 to FChilds.FCount - 1 do begin
			...
			if (aChild.Get_NodeType = NODE_ELEMENT) and (aChild.Get_NodeNameID = aNameID) then begin
				...
				try
					...
					// Исключительн? ситуац? може?возникнуть ?то?случае,
					// если произойдет сбой ?функци?VarSameValue.
					// Иным?словам?- если значен? нель? сравнивать.
				end;
			end
		end;
	end;
...
// на вход?- первый символ имен?
// на выходе - первый символ, которы?не являет? допустимым для имен
...
// на вход?- первый символ числ?
// на выходе - первый символ, которы?не являет? допустимым для чисе?
...
// на вход?- первый символ числ?
// на выходе - первый символ, которы?не являет? допустимым для
//  щестнадцатиричны?чисе?
...
// на вход? открывающая кавычк?
// на выходе: символ, следующи?за закрывающе?кавычкой
...
// на вход? первый символ текста
// на выходе: символ, следующи?за последни?символом ограничите?
