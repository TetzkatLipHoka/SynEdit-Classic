...
// TSynSpellCheck 1.50
//
// Copyright © 2000-2006 Jacob Dybala a.k.a. "m3Rlin". All rights reserved.
//
// E-Mail: m3Rlin@m3Rlin.org
// WWW   : http://www.m3Rlin.org/ SynSpellCheck Home
//
// Elf hash algorithm
//   Copyright © 1998-2002 Scalabium
//   <http://www.scalabium.com/faq/dct0136.htm>
// SoundEx algorithm
//   Copyright © 1995-2001 Borland Software Corporation
// Metaphone Phonetic Hash Algorithm
//   Copyright © Tom White <wcstom@yahoo.com>
// Word differences algorithm JHCMP...
//   Copyright © Josef Hampl
//
// Created : Jan-10-2002
// Modified: Sep-15-2006
////////////////////////////////////////////////////////////////////////////////
...
//
// Available dictionaries - Check http://www.m3rlin.org/wordpress/?page_id=29
// for the most current list of available dictionaries.
////////////////////////////////////////////////////////////////////////////////
// Changes:
//
// 1.50
...
//     rather large memory leak. Fixed. Submitted by Stefan Ascher.
...
//   + New dictionaries: German (by Stefan Ascher) and Russian.
//
// 1.18
...
type
  ...
  public
    function GetWordAtRowColEx(XY: TPoint; cIdentChars: TSynIdentChars;
      OverrideHighlighterChars: Boolean): UnicodeString;
  ...
  public
    ...
    { Procedures }
    procedure MetaPhone(A: UnicodeString; lg: Integer; var Res: UnicodeString);
    procedure Execute;
  published
    { Properties }
    property InString: UnicodeString read sIn write sIn;
    property OutLength: Integer read LengthVar write SetLength default 20;
    property OutString: UnicodeString read sOut write sOut;
  end;
  ...
  { Procedure types }
  TOnAddWord = procedure(Sender: TObject; AWord: UnicodeString) of object;
  TOnCheckWord = procedure(Sender: TObject; AWord: UnicodeString;
    ASuggestions: TStringList; var ACorrectWord: UnicodeString; var AAction: Integer;
    const AUndoEnabled: Boolean = True) of object;
  TOnCorrectWord = procedure(Sender: TObject; AWord, ACorrectWord: UnicodeString)
    of object;
  TOnSkipWord = procedure(Sender: TObject; AWord: UnicodeString; ASkipAll: Boolean)
    of object;
  ...
  private
    ...
    FUnderlineStyle    : TUnderlineStyle;
    { Functions }
    function FindWord(sWord: UnicodeString): Integer;
    ...
    function JHCMPDiffCount(const Str1, Str2: UnicodeString): Longint; overload;
    function JHCMPDiffCount(const Str1, Str2: UnicodeString;
    ...
    function JHCMPIsSimilar(const Str1, Str2: UnicodeString;
    ...
    procedure SetApostrophes(const Value: AnsiString);
  ...
  public
    ...
    function Ansi2Ascii(const sString: Ansistring): Ansistring;
    function CheckWord(Word: UnicodeString): Boolean;
  ...
  {$endif}
    ...
    function GetWordFromASCIIWord(sWord: UnicodeString): UnicodeString;
    function IsDictWord(Word: UnicodeString): Boolean;
    function IsSkipWord(Word: UnicodeString): Boolean;
    function RemoveEditor(AEditor: TCustomSynEdit): Boolean;
    { Procedures }
    procedure AddDiacritic(Progress: TProgressBar);                             //Fiala
    procedure AddDictWord(Word: UnicodeString);
    procedure AddDictWordList(WordList: TStringList);
    procedure AddSkipWord(Word: UnicodeString);
    procedure AddSkipWordList(WordList: TStringList);
    procedure ClearDictWords;
    procedure ClearSkipWords;
    procedure CloseDictionary;
    procedure DeleteDictWord(Word: UnicodeString);
    procedure DeleteSkipWord(Word: UnicodeString);
    procedure FixLists;
  ...
  {$endif}
    ...
    // Modified by Administrator 2007-2-2 0:38:00
  ...
  published
    ...
    property Apostrophes: AnsiString read FApostrophes write SetApostrophes;
...
function TrimEx(const sWord: UnicodeString; const chChar: WideChar): UnicodeString;
...
begin
{$ifdef SYN_WIN32}
  // Modified by Administrator 2007-2-2 0:37:42
  SetCurrentDir(ExtractFileDir(ParamStr(0)));
  if FDictPath <> '' then
    Result := ExpandPath(IncludeTrailingBackslash(FDictPath))
  else
...
begin
{$ifdef SYN_WIN32}
  // Modified by Administrator 2007-2-2 0:37:48
  SetCurrentDir(ExtractFileDir(ParamStr(0)));
  if FUserDictPath <> '' then
    Result := ExpandPath(IncludeTrailingBackslash(FUserDictPath))
  else
    Result := IncludeTrailingBackslash(GetDefaultDictionaryDir);
{$else}
  if FUserDictPath <> '' then
...
procedure TMetaphone.Metaphone(A: UnicodeString; lg: Integer; var Res: UnicodeString);
var
  iI: Integer;
begin
  for iI := 1 to lg do
...
begin
  ...
  while FirstLine <= LastLine do begin
    ...
    while CX < Length(Editor.Lines[FirstLine - 1]) do begin
      CurrentXYTPoint   := Point(CX, FirstLine);
      CurrentXY.Column  := CurrentXYTPoint.x;
      CurrentXY.Row  := CurrentXYTPoint.y;
      CurrentWord := TSynEditEx(Editor).GetWordAtRowColEx(CurrentXYTPoint,
        FSynSpellCheck.FIdentChars, True);
      TP := Editor.RowColumnToPixels(CurrentXY);
      if TP.X > ACanvas.ClipRect.Right - ACanvas.ClipRect.Left then
        Break;
      if Assigned(Editor.Highlighter) then begin
        if Editor.GetHighlighterAttriAtRowCol(BufferCoord(CurrentXY.Column, CurrentXY.Row)
          {CurrentXY}, sToken, Attri) = False then
...
function TSynSpellCheck.Ansi2Ascii(const sString: AnsiString): AnsiString;
var
  iI: Integer;
begin
  Result := sString;
...
begin
  ...
  if FindFirst(GetDictionaryDir + Language + '.?-?-?.dic', faAnyFile,
    ...
    if Pos('.user.', srDict.Name) = 0 then
      tslTemp.Add(LowerCase(srDict.Name));
...
function TSynSpellCheck.GetWordFromASCIIWord(sWord: UnicodeString): UnicodeString;
var
  iI, iJ, iLength: Integer;
  sLower, sTemp  : string;

  function CorrectCase(const AsWord: UnicodeString; const Word: UnicodeString): UnicodeString;
  var
    s1, s2, s3, s4: UnicodeString;
    iX: Integer;
  begin
    s1 := UpperCase(AsWord);
    s2 := LowerCase(AsWord);
...
function TSynSpellCheck.JHCMPDiffCount(const Str1, Str2: UnicodeString;
...
function TSynSpellCheck.JHCMPIsSimilar(const Str1, Str2: UnicodeString;
...
procedure TSynSpellCheck.AddDictWord(Word: UnicodeString);
var
  AWordItem: PWordRec;
...
procedure TSynSpellCheck.AddSkipWord(Word: UnicodeString);
begin
  if Trim(Word) <> '' then
    FSkipList.Add(LowerCase(Word));
end;
...
function TSynSpellCheck.CheckWord(Word: UnicodeString): Boolean;
var
  iI: Integer;
begin
  Word := Trim(Word);
  ...
  if sscoIgnoreWordsWithNumbers in FOptions then
    for iI := 1 to Length(Word) do
      if AnsiChar( Word[iI] ) in ['0'..'9'] then
        begin
        Result := True;
        Exit;
        end;
  ...
  //////////////////////////////////////////////////////////////////////////////
  Result := (FindWord(LowerCase(Word)) > -1);
  if not Result and (FSkipList.IndexOf(LowerCase(Word)) <> -1) then
    Result := True;
end;
...
procedure TSynSpellCheck.DeleteDictWord(Word: UnicodeString);
begin
  Dispose(PWordRec(FWordList.Items[FindWord(LowerCase(Word))]^));
end;

procedure TSynSpellCheck.DeleteSkipWord(Word: UnicodeString);
begin
  with FSkipList do
    Delete(IndexOf(LowerCase(Word)));
end;

function TSynSpellCheck.IsDictWord(Word: UnicodeString): Boolean;
begin
  Result := (FindWord(LowerCase(Word)) <> -1);
end;

function TSynSpellCheck.IsSkipWord(Word: UnicodeString): Boolean;
begin
  Result := (FSkipList.IndexOf(LowerCase(Word)) <> -1);
end;
...
begin
  for iI := 0 to FSkipList.Count - 1 do
    FSkipList.Strings[iI] := LowerCase(FSkipList.Strings[iI]);
end;
    ...
    for iI := 0 to FWordList.Count - 1 do
      if (TWordRec(FWordList.Items[iI]^).Hash = sHash) and (Abs(iLength - Length(TWordRec(FWordList.Items[iI]^).Word)) < 2) then begin
        ...
        if sscoMaintainCase in FOptions then begin
          ...
          else if UpperCase(sWord[1])[1] = Word[1] then
            sWord[1] := UpperCase(sWord[1])[1];
        end;
...
begin
  ...
  for iI := FCacheArray[Ord(chFirst)][0] to FCacheArray[Ord(chFirst)][1] do begin
    ...
    if JHCMPIsSimilar(sLower, sWord, MaxDiffCount, Differences) then begin
      if UpperCase(Word) = Word then
        sWord := UpperCase(sWord)
      else if UpperCase(Word[1])[1] = Word[1] then
        sWord[1] := UpperCase(sWord[1])[1];
      Similar.Add(sWord);
    end;
  end;
...
var
  ...
  procedure AddNewWord(sWord: UnicodeString; IsUser: Boolean);
  begin
      New(AWordItem);
...
begin
  // Modified by Administrator 2007-2-2 0:37:24
  if FOpenDictionary then
  begin
    SaveSkipList(GetUserDictionaryDir+FLanguage.Name + '.SkipWord.dic');
    SaveUserDictionary;
  end;
  ...
  if FUseUserDictionary then begin
    ...
    // Modified by Administrator 2007-2-2 0:40:19
    if FileExists(FUserDictPath+FLanguage.Name + '.SkipWord.dic') then
      LoadSkipList(FUserDictPath+FLanguage.Name + '.SkipWord.dic')
  end;
...
function TSynSpellCheck.FindWord(sWord: UnicodeString): Integer;
{var
  iI, iHash: Integer;
begin
  Result := -1;
...
begin
  ...
  with TSynEditEx(FEditor) do begin
    BlockBegin := BufferCoord(WordStartEx(FWhiteChars).X, WordStartEx(FWhiteChars).Y);
    BlockEnd   := BufferCoord(WordEndEx(FWhiteChars).X, WordEndEx(FWhiteChars).Y);
  end;
end;
...
var
  ...
  CaretXYTPoint: TPoint;

  function InternalCheckWord(Word: UnicodeString): Boolean;
  var
    iAction     : Integer;
    sCorrectWord: UnicodeString;
  begin
    Result := True;
    if not CheckWord(LowerCase(Word)) then begin
      ...
      if Assigned(FOnCheckWord) then begin
        ...
        case iAction of
          ...
          ACTION_UNDO: begin
            with TSynEditEx(FEditor) do begin
              Undo;
              CaretXY := BufferCoord(pLastWord.X, pLastWord.Y);
              if sscoGoUp in FOptions then
                CaretXY := BufferCoord(NextWordPosEx(FIdentChars, FWhiteChars).X,
                  NextWordPosEx(FIdentChars, FWhiteChars).Y)
              else
                CaretXY := BufferCoord(PrevWordPosEx(FIdentChars, FWhiteChars).X,
                  PrevWordPosEx(FIdentChars, FWhiteChars).Y);
              bUndoEnabled := False;
            end;
          end;
        end;
...
begin
  ...
  with TSynEditEx(FEditor) do begin
    ...
    while pNextWord.X > 0 do begin
      ...
      //////////////////////////////////////////////////////////////////////////
      while Trim(GetWordAtRowColEx(Point(CaretXY.Char, CaretXY.Line), FIdentChars, True)) = '' do begin
        { Just move to next word }
        if sscoGoUp in FOptions then
          pNextWord := PrevWordPosEx(FIdentChars, FWhiteChars)
        else
          pNextWord := NextWordPosEx(FIdentChars, FWhiteChars);
        CaretXY := BufferCoord(pNextWord.X, pNextWord.Y);
      ...
      if pNextWord.X = 0 then
        Break;
      sWord := GetWordAtRowColEx(Point(CaretXY.Char, CaretXY.Line), FIdentChars, True);
      //////////////////////////////////////////////////////////////////////////
...
procedure TSynSpellCheck.SetApostrophes(const Value: AnsiString);
var
  chChar: AnsiChar;
  iI    : Integer;
begin
  ...
  for chChar := #128 to #255 do                                                      //Fiala
    if IsCharAlphaNumericA(chChar) then
...
begin
  ...
  with TSynEditEx(FEditor) do begin
    ...
    oldCaret := Point(CaretXY.Char, CaretXY.Line);
    oldTopLine := TopLine;
    CaretXY := BufferCoord{Point}(1, 1);
  ...
  Screen.Cursor := FCursor;
  with FEditor do begin
    CaretXY := BufferCoord(oldCaret.X, oldCaret.Y);
    TopLine := oldTopLine;
    BlockBegin := CaretXY;
    BlockEnd   := BlockBegin;
  end;
...
procedure TSynSpellCheck.LoadFromIni(IniFile: TCustomIniFile;
  Section: string);
var
  I, Count : Integer;
  Option :SynSpellCheckOptions;
  OptionStr : string;
begin
  with IniFile do
  begin
    Algorithm := HashAlgorithms(ReadInteger(Section, 'Algorithm', Integer(Algorithm)));
    Apostrophes := ReadString(Section, 'Apostrophes', Apostrophes);
    DictionaryPath := ReadString(Section, 'DictionaryPath', DictionaryPath);
    fDictionary := ReadString(Section, 'Language', 'english');
    HashLength := ReadInteger(Section, 'HashLength', HashLength);
    Count := ReadInteger(Section, 'CheckAttribCount', 0);
    for i := 0 to Count - 1 do
      CheckAttribs.Add(ReadString(Section, 'CheckAttrib'+IntToStr(i), ''));
    for Option := sscoAutoSpellCheck to sscoTrimApostrophes do
      OptionStr := OptionStr + IntToStr(Integer(Option in Options));
    OptionStr := ReadString(Section, 'Options', OptionStr);
    for I := 1 to Length(OptionStr) do
      if (OptionStr[i] = '1') then
        FOptions := FOptions + [SynSpellCheckOptions(I-1)];
    UnderlineStyle := TUnderlineStyle(ReadInteger(Section, 'UnderlineStyle', Integer(UnderlineStyle)));
    UserDirectory := ReadString(Section, 'UserDirectory', UserDirectory);
    UseUserDictionary := ReadBool(Section, 'UseUserDictionary', UseUserDictionary);
  end;
end;

procedure TSynSpellCheck.SaveToIni(IniFile: TCustomIniFile;
  Section: string);
var
  I : Integer;
  Option :SynSpellCheckOptions;
...
begin
  with IniFile do
  begin
    WriteInteger(Section, 'Algorithm', Integer(Algorithm));
    WriteString(Section, 'Apostrophes', Apostrophes);
    WriteString(Section, 'Language', Dictionary);
    WriteString(Section, 'DictionaryPath', FDictPath);
    WriteInteger(Section, 'HashLength', HashLength);
    WriteInteger(Section, 'CheckAttribCount', CheckAttribs.Count);
    for i := 0 to CheckAttribs.Count - 1 do
      WriteString(Section, 'CheckAttrib'+IntToStr(i), CheckAttribs[i]);
    for Option := sscoAutoSpellCheck to sscoTrimApostrophes do
      OptionStr := OptionStr + IntToStr(Integer(Option in Options));
    WriteString(Section, 'Options', OptionStr);
    WriteInteger(Section, 'UnderlineStyle', Integer(UnderlineStyle));
    WriteString(Section, 'UserDirectory', FUserDictPath);
    WriteBool(Section, 'UseUserDictionary', UseUserDictionary);
  end;
end;
...
function TSynEditEx.GetWordAtRowColEx(XY: TPoint;
  cIdentChars: TSynIdentChars; OverrideHighlighterChars: Boolean): UnicodeString;
var
  Line: string;
  ...
  if (XY.Y >= 1) and (XY.Y <= Lines.Count)  then begin
    ...
    if (XY.X >= 1) and (XY.X <= Len + 1) then begin
      if Assigned(Highlighter) and not OverrideHighlighterChars then
        IdChars := Highlighter.AdditionalIdentChars
      else
        IdChars := cIdentChars;
      Stop := XY.X;
...
{$ELSE}
      MultiPos := 0;
      // find next "whitespace" if current char is an IdentChar
      if Line[CX] in AdditionalIdentChars then
        CX := StrScanForCharInSet(Line, CX, cWhiteChars);
{$ENDIF}
